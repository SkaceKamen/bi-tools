...
async function loadAllFromCategory(category: string) {
	...
	const scripts = listData
		.substring(listStart + T_LIST_START.length, listEnd)
		.trim()
		.replace(/\r/g, '')
		.split('\n')
	...
	const result = await fetch(
		'https://community.bistudio.com/wiki/Special:Export/',
		{
			...
			body: `pages=${scripts.join(
				'%0D%0A'
			)}&curonly=1&wpDownload=1&title=Special%3AExport%2F`,
			method: 'POST',
		}
	).then((res) => res.text())
...
async function main() {
	fs.writeFileSync(
		join(__dirname, '..', 'exports', 'operatorsExport.xml'),
		await loadAllFromCategory('Scripting_Commands')
	)
	console.log('Saved', 'operatorsExport.xml')
	fs.writeFileSync(
		join(__dirname, '..', 'exports', 'functionsExport.xml'),
		await loadAllFromCategory('Functions')
	)
