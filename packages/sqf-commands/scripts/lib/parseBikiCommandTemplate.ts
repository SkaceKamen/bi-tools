...
export const parseBikiCommandTemplate = (
	code: string,
	name: string,
	isFunction = false
): CommandOrFunctionInfo => {
	...
	const parseTemplateParameter = (node: Parser.Token) => {
		...
		if (versionMatch) {
			...
			compatibility[index].version = innerText(node.childNodes?.[1]).trim()
	...
	const syntaxesResult = Object.values(syntaxes).map((s) => {
		const code = s.code
		const match =
			!isFunction &&
			new RegExp(`([^\\s]+\\s)?${escapeRegExp(name)}\\s*(.*)`, 'i').exec(code)

		const args = Object.values(s.args).map((a) => {
			return {
				...a,
				position: (match && match[1]
					? match[1].toLowerCase().includes(a.name.toLowerCase())
						? 'left'
						: 'right'
					: 'right') as 'left' | 'right',
			}
