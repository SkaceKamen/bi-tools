...
import { WikiDocumentation } from '../../src/types'
import { parseBikiCommandTemplate } from './parseBikiCommandTemplate'

export function parseBikiExport(
	doc: string,
	type: 'function' | 'command',
	results = {} as Record<string, WikiDocumentation>
) {
	...
	for (const i in pages) {
		...
		if (parseInt(page.childNamed('ns')?.val ?? '-1') === 0) {
			...
			const data = parseBikiCommandTemplate(text, title, type === 'function')
